...
class LaurentCoreImport {
	...
	public function set_import_url() {
		$params = LaurentCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
			...
			if (isset($_POST) || !empty($_POST) || isset($_POST['options']['demo'])) {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'eltdf_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (laurent_core_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'laurent-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!LaurentCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									laurent_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'laurent-core'));
									exit;
								}
							}
			...
			laurent_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'laurent-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $laurent_widget_id => $laurent_widget_data) {
				update_option('widget_' . $laurent_widget_id, $laurent_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'laurent-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$laurent_sidebars = get_option( "sidebars_widgets" );
		unset( $laurent_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$laurent_sidebars = array_merge( (array) $laurent_sidebars, (array) $data['sidebars'] );
			unset( $laurent_sidebars['wp_inactive_widgets'] );
			$laurent_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $laurent_sidebars );
			$laurent_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $laurent_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_custom_sidebars( $file ) {
		...
		if($options) {
			$results = update_option('eltdf_sidebars', $options);
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(LAURENT_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(LAURENT_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'laurent-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'laurent-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'laurent-core'));
			}
		}
		if(laurent_elated_is_plugin_installed('elementor')){
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_laurent_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'laurent-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'laurent-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'laurent-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'laurent-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'laurent-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( LAURENT_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(laurent_core_is_woocommerce_installed()) {
		...
		$laurent_import = new WP_Import();
		set_time_limit( 0 );

		$laurent_import->fetch_attachments = $attachments;
		$returned_value                  = $laurent_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'laurent-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'laurent-core' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'laurent-core'));
					exit;
				}
			}
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'laurent-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'laurent-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( LAURENT_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( LAURENT_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function import_get_demo_urls( $folder ) {
		...
		$domain_url = defined( 'LAURENT_ELATED_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.' . LAURENT_ELATED_PROFILE_SLUG . '-themes.com/' : '';
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return LAURENT_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'eltdf_cd_import_nonce' ) ) {
				...
				$html = laurent_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					laurent_core_ajax_status( 'success', '', $html);
				} else {
					laurent_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'laurent-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = LaurentCoreSystemInfoPage::get_instance()->get_system_info();
...
LaurentCoreImport::get_instance();